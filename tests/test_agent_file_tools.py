...
def test_list_and_describe_project_files(project_files, agent_context):
    ...
    flat = project_files.describe_file("runs/run_a/work/case_flat.csv")["data"]
    assert (flat["row_count"], flat["columns"][:3], len(flat["rows"])) == (3, ["event_idx", "contingency", "from_bus"], 3)
    raw = project_files.describe_file("runs/run_a/work/case.raw")["data"]
    assert [(item["name"], item["record_count"]) for item in raw["sections"]] == [("BUS", 2), ("LOAD", 1), ("BRANCH", 1), ("TRANSFORMER", 1), ("AREA", 1)]
    xml = project_files.describe_file(str(agent_context.run("run_a") / "work/input.xml"))["data"]
    assert xml["field_count"] == 8
    log = project_files.describe_file("runs/run_a/logs/run.log")["data"]
    assert (log["kind"], log["line_count"], log["rows"][0]) == ("text", 10, {"line": 1, "text": "line 1"})


def test_query_table_filters_sorts_projects_and_pages(project_files):
    """Filters, sorting, column selection, and paging work the same for CSV, text tables, and RAW sections."""
    flat = "runs/run_a/work/case_flat.csv"
    over = project_files.query_table(flat, filters=[{"column": "loading_percent", "op": ">", "value": 80}], sort_by="loading_percent", descending=True, columns=["event_idx", "loading_percent"])
    ...
    page = project_files.query_table(flat, offset=1, limit=1)["data"]
    assert (page["rows"][0]["contingency"], page["total_matching"], page["next_offset"]) == ("line outage", 3, 2)
    assert project_files.query_table(flat, filters=[{"column": "contingency", "op": "in", "value": ["base", "island"]}])["data"]["total_matching"] == 2
    assert project_files.query_table(flat, filters=[{"column": "contingency", "op": "contains", "value": "OUT"}])["data"]["total_matching"] == 1
    assert project_files.query_table(flat, columns=["nope"])["error"]["code"] == "UNKNOWN_COLUMN"
    assert project_files.query_table(flat, filters=[{"column": "loading_percent", "op": "~", "value": 1}])["error"]["code"] == "INVALID_FILTER"
    vmag = project_files.query_table("runs/run_a/work/vmag_mm.txt", filters=[{"column": "min_value", "op": "<", "value": 0.92}])["data"]
    assert [row["bus_id"] for row in vmag["rows"]] == ["2"]
    raw = project_files.query_table("runs/run_a/work/case.raw", table="bus", sort_by="BASKV")["data"]
    assert [row["NAME"] for row in raw["rows"]] == ["BETA, TWO", "ALPHA 1"]
    assert project_files.query_table("runs/run_a/work/case.raw")["error"]["code"] == "RAW_SECTION_REQUIRED"
    assert project_files.query_table("runs/run_a/logs/run.log")["error"]["code"] == "NOT_A_TABLE"


def test_parquet_tables_are_queryable(project_files, agent_context):
    """Parquet files, such as the event index, are read with the same filters."""
    ...
    result = project_files.query_table(str(path), filters=[{"column": "loading_percent", "op": ">=", "value": 90}], sort_by="event_idx")["data"]
    assert result["rows"] == [{"event_idx": 1, "loading_percent": 120.0}, {"event_idx": 2, "loading_percent": 90.0}]
    assert project_files.describe_file(str(path))["data"]["row_count"] == 3


def test_text_lines_and_document_fields(project_files):
    """Lines are numbered and paged; JSON and XML documents become rows of path and value."""
    lines = project_files.read_text_file("runs/run_a/logs/run.log", offset=8, limit=5)["data"]
    assert lines["rows"] == [{"line": 9, "text": "line 9"}, {"line": 10, "text": "line 10"}]
    assert (lines["total_matching"], lines["truncated"]) == (10, False)
    xml = {row["path"]: row["value"] for row in project_files.read_document("runs/run_a/work/input.xml")["data"]["rows"]}
    assert xml["Configuration/Contingency_analysis/FullBranchN1"] == "true"
    assert xml["Configuration/Powerflow/qlim"] == "false"
    manifest = {row["path"]: row["value"] for row in project_files.read_document("runs/run_a/manifest.json")["data"]["rows"]}
    assert manifest["$.command[3]"] == "ca.x"
    assert project_files.read_document("runs/run_a/logs/run.log")["error"]["code"] == "NOT_A_DOCUMENT"
...
def test_repeated_xml_tags_are_numbered(project_files, agent_context):
    ...
    rows = project_files.read_document(str(path))["data"]["rows"]
...
def test_paths_stay_inside_projects_and_the_session(project_files, agent_context, tmp_path):
    ...
    assert project_files.describe_file(str(outside))["error"]["code"] == "PATH_OUTSIDE_PROJECTS"
    link = agent_context.run("run_a") / "work/linked.csv"
    link.symlink_to(outside)
    assert project_files.query_table(str(link))["error"]["code"] == "PATH_OUTSIDE_SESSION"
    assert project_files.read_text_file("runs/run_a/work/missing.txt")["error"]["code"] == "FILE_NOT_FOUND"
    assert project_files.describe_file("")["error"]["code"] == "INVALID_PATH"
    results = agent_context.directory / "results"
    results.mkdir()
    (results / "T9.csv").write_text("line_id,max\n1,120\n2,80\n")
    saved = project_files.query_table(str(results / "T9.csv"), filters=[{"column": "max", "op": ">", "value": 100}])
...
def test_memory_guard_asks_for_pages(project_files, monkeypatch):
    ...
    refused = project_files.query_table("runs/run_a/work/case_flat.csv", limit=0)
    assert refused["error"]["code"] == "QUERY_TOO_LARGE"
    assert project_files.query_table("runs/run_a/work/case_flat.csv", limit=2)["data"]["returned"] == 2
