...
def _table_index(table: ParsedTable | None) -> dict[tuple[object, object, str, str], dict[str, object]]:
    ...
    for row in table.rows:
        indexed.setdefault(branch_key(row), row)
    return indexed


def branch_key(row: Mapping[str, object]) -> tuple[object, object, str, str]:
    """Identify a monitored facility without merging circuits or sections."""
    return (
